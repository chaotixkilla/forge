...
## Gotchas
- **Delegates its evidence-gathering to the `gather` skill (phases 1 and 3),** which pulls glossary, standing conventions, and behavioral invariants — perf budgets, a11y/security baselines, tenancy rules — that the spec must respect. spec declares no knowledge configuration of its own: `gather` owns the `tools.knowledge` prerequisite and, if it is unset, guides you through init:knowledge or degrades to spec'ing without that grounding (weaker, more assumption-laden).
