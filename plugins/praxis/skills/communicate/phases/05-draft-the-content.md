This is where the artifact gets written — but "write it well" is a dozen judgment calls a cold executor would each resolve differently, so this phase routes each to the rule that pins it rather than leaving them to instinct. The frame, the tier, the form, and the sourced requirement set from [derive-and-source](04-derive-and-source.md) are fixed inputs now; the work here is producing text that lands them on the reader. The order below is the drafting order: skeleton first, then fill, then the craft passes that make it read for *this* reader.

**Draft from the requirement set, not from the session.** Every entry goes in — an obtained requirement as text, an unobtained one as the declaration [source-or-declare](../rules/source-or-declare.md) pins for its disposition. Reaching past the set for something that merely felt relevant is how unsourced material re-enters after the phase that existed to keep it out.
...
## Place the visuals the requirement set already assigned

Which requirements became a table, a diagram, or a chart was decided in [derive-and-source](04-derive-and-source.md) against the content's shape; drafting places them, it does not re-open the choice. Put each where the reader meets the requirement it discharges, not in a figures section at the end. Render diagrams to the notation, size, edge-labeling, and declared-elision bars in [diagram-legibility](../rules/diagram-legibility.md) — a diagram that ships without its elision line makes a completeness claim the artifact cannot support. Two things come from [when-a-visual-is-owed](../rules/when-a-visual-is-owed.md) at placement time: the **one-home rule** — where a visual carries a requirement, the prose points at it rather than saying it again — and the **chart fallback**, since a requirement assigned a chart with no charting capability present is placed as the figures table that rule prescribes, carrying the claim, the axes' labels and units, and the n.
