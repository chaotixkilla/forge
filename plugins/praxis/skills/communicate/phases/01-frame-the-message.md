...
The takeaway is a claim about something real — a decision, a change, a state — and you cannot state it from recollection. Locate the thing itself: recruit the **repository** and **code** explorers for what changed and why in the tree, and the **knowledge-base** explorer for the settled context the artifact rests on (this is the direct doc-context read that keeps `tools.knowledge` on this skill — reading, not investigating); or, without fan-out, perform those reads inline before continuing. If knowledge is unavailable, degrade to what the session already holds and note where the framing rests on unconfirmed recollection.
