...
Activated by `--publish`, referenced from [deliver-and-route](../phases/07-deliver-and-route.md).

Base behavior: [deliver-and-route](../phases/07-deliver-and-route.md) produces the artifact and returns it (the record). This module additionally publishes it as a durable, team-facing document and returns its canonical location. Deletion test: remove it and deliver-and-route still produces and returns the artifact; the durable publish is additive — so it is a module.
...
- **Re-apply the clean-export bar** — publishing puts the artifact somewhere durable and often public, so [clean-export](../rules/clean-export.md) re-applies at the handoff: every internal-process reference is already stripped in [tighten-and-verify](../phases/06-tighten-and-verify.md), and this is the last checkpoint before it becomes a permanent record. The port publishes faithfully and adds no process metadata of its own.
