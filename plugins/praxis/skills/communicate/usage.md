...
## Gotchas
- **communicate needs no delivery configuration of its own.** Producing and returning the artifact is ambient. The communication capability (reached by `--notify`) and the artifacts capability (reached by `--publish`) are delegated to their ports, which own `tools.communication` / `tools.artifacts`. If a backend isn't configured, the port guides you through `init` (or blocks), and communicate degrades: it returns the finished artifact for you to send or publish by hand. Only `tools.knowledge` is communicate's own, and it degrades to what the session already holds.
