...
`(basis: ratified by the maintainer, 2026-09-02, from the reported gap that praxis tests the surface it touched but not the assumptions the work rests on. The seam is one-directional and was open: plan produces falsifiable assumptions with their consequences, and nothing in this skill read them, so a design's riskiest premises past the first were surfaced and then never tested. Taking the breakage as the expected result is what keeps this from becoming a second, vaguer risk pass — the case has a defined outcome because the design already stated one.)`
