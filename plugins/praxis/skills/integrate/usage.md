...
- You want the landing to match how this team actually works — its branch model, merge style, and message conventions — instead of a generic ritual imposed on the repo.
...
- **It follows the team's flow, not a house ideal.** Merge style, branch model, and message convention are read from the repo's actual convention first, then any house rule, before anything is imposed — where they genuinely conflict, integrate surfaces the choice rather than picking one.
