...
`--publish` — publish the finished report as a team-facing document through the artifacts capability instead of returning it inline.
...
- **The report is a clean export.** `--publish` publishes a team-facing document — the findings, sources, and confidence for a human reader — carrying no internal tool calls, agent/phase mechanics, or praxis process; the machinery stays out of the deliverable.
...
- **deep-research needs no configuration of its own.** Web search and source-fetch are ambient; the org-internal knowledge lane is reached through the `gather` port, which routes it to the `knowledge` port (the prerequisite's owner), so it is present only when a knowledge backend is configured and degrades cleanly when it isn't; publishing (`--publish`) is delegated to the `publish-artifact` port. Each doer owns its prerequisite, so deep-research declares none.
