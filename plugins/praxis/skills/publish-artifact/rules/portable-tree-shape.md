...
The carve is reproducible because the primary gate is a **count** and the override keys off **structural signals** (a section's own nested outline; dominant length), not a quality adjective. `(basis: derived from the round-trippable-tree goal in the design seed (skill-seeds: portable-tree-shape) — a count-plus-structural-signal gate is what makes the carve reproducible across cold runs, the defect a bare "stands alone as its own topic" left open.)`
