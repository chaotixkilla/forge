...
Below the ladder — blog posts, talk summaries, top-voted Q&A answers, tutorials, LLM-generated explainers — is not your tier, no matter how confidently written. That is the *plugin-community-practices* explorer's territory. If the official sources are thin and the real answer lives only there, that is itself your finding — "the standard is silent on X; only community sources answer it" — report the silence; do not launder a blog into the authoritative tier.
