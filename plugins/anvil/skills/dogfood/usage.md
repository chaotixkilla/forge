...
- A run that succeeds can still be carrying dead weight. The challenge phase's economy-skeptic critic reads the same logs for the opposite defect: files never opened, citations never followed, prose whose removal would have changed nothing. This is the one lens the static authoring gate cannot run as well as dogfood can, because only a real run shows what went untouched.
